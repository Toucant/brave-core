...
void BraveCosmeticResourcesTabHelper::GetUrlCosmeticResourcesOnUI(
    content::RenderFrameHost* render_frame_host, const std::string& url,
  ...
  for (auto i = resources->GetList().begin();
    ...
    if (!main_frame)
      return;
    CSSRulesRoutine(url, resources_dict, render_frame_host);
  }
}
  ...
  g_brave_browser_process->ad_block_service()->GetTaskRunner()->
      PostTaskAndReplyWithResult(FROM_HERE,
          ...
          base::BindOnce(&BraveCosmeticResourcesTabHelper::
            ...
            render_frame_host, url.spec(), main_frame));
  if (!main_frame)
    return;
